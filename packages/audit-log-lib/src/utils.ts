...
async function buildExcelBuffer(
  ...
  logs.forEach((log) => {
    ...
    const row = sheet.addRow(flatLog);
  ...
  sheet.columns.forEach((column) => {
    let maxLength = column.header ? column.header.length * 1.5 : 10;
...
export async function downloadLogs(
  ...
  if (format === "json") {
    const blob = new Blob([JSON.stringify(readableLogs, null, 2)], {
